								...
								<div class="card-body table-responsive p-0">
									...
									<table id="example1" class="table table-bordered table-striped">
										...
										<%-- <c:url var="deleteLink" value="/employee/deleteEmployee" >
											<c:param name="empNumber" value="${employees.empNumber}" ></c:param>
										</c:url> --%>
											<tr>
												<td>${employees.empNumber}</td>
												<td>${employees.firstName}</td>
												<td>${employees.lastName}</td>
												<td>${employees.gender}</td>
												<td>${employees.edution}</td>
												<td>${employees.deptDTO.deptName}</td>
												<td>${employees.married}</td>
												
												<td>
													<button class="btn btn-primary" data-item-id="1"
														data-toggle="modal" data-target="#modal-default"
														onclick="populate(${employees.serialNumber}, '${employees.empNumber}', '${employees.firstName}', 
														'${employees.middleName}', '${employees.lastName}', '${employees.gender}', '${employees.edution}',
														'${employees.dept_id}', '${employees.married}', '${employees.address}',
														'${employees.password}', '${employees.isAdmin}' )">Update
													</button>
												</td>
												<td>
												<button class="btn btn-danger delete-item" data-item-id="1"
													onclick="deleteEmployee(${employees.empNumber})">Delete
												</button>
												</td>
											</tr>
										</c:forEach>
        ...
        <div class="modal-dialog">
          <div class="modal-content bg-warning">
            ...
            <div class="modal-footer justify-content-between">
              ...
              <button type="button" class="btn btn-outline-dark"   onclick="confirmDeleteEmployee()">Yes</button>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
